...
const updateModalData = async (req, res) => {
  try {
    ...
    const updatedModalData = await ModalData.findByIdAndUpdate(
      id,
      modalDataUpdates
    );
    if (!updatedModalData) {
    ...
    res
      .status(500)
      .json({ success: false, error: "Update failed", details: error.message });
  }
};
...
const deleteModalData = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Delete failed", details: error.message });
  }
};
...
const getCatgoryNews = async (req, res) => {
  try {
